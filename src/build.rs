...
use crate::utils::{copy_dir_all, Article, ArticleAsset, FrontMatter, OUTPUT_FOLDER_PATH};

pub fn build(working_path: &str) {
    rm_rf::ensure_removed(OUTPUT_FOLDER_PATH).expect("Unable to delete dist");
    ...
    copy_dir_all(
        absolute_working_path.as_path().join("public"),
        Path::new(OUTPUT_FOLDER_PATH).join("public"),
    )
    .expect("Unable to copy public \"folder\"");
